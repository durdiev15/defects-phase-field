...
def compute_properties_1d(eta, dx, omega, gamma, kappa):
    ...
    return sigma_calculated, width_calculated
...
def plot_fields_1d(output_dir, n, x_nm, phi_phys, rho_phys, C_vo_phys, C_dop_phys, eta0, eta1):
...
def update_concentration_semi_implicit(C, mu, M, dt, dx, LU, pivots, D_stab):
    """Semi-implicit update using pre-factorized LU matrix."""
    # 1. Calculate explicit non-linear flux (Phase-Field Physics)
    dmu_dx = (mu[1:] - mu[:-1]) / dx
    M_mid = 0.5 * (M[1:] + M[:-1])
    J_x = -M_mid * dmu_dx
    ...
    # 2. Subtract explicit stabilizer
    lap_C = laplacian_neumann_1d(C, dx)
    RHS = C + dt * minus_div_J - dt * D_stab * lap_C
    
    # 3. Solve implicit system: (I - dt*D*Lap) * C_new = RHS
    C_new = torch.linalg.lu_solve(LU, pivots, RHS.unsqueeze(1)).squeeze(1)
    return C_new
def build_semi_implicit_lu(Nx, dx_dimless, dt, D_stab):
    """Pre-computes LU factorization for the semi-implicit transport step."""
    alpha = (dt * D_stab) / (dx_dimless**2)
    A = torch.zeros((Nx, Nx), dtype=dtype, device=device)
    idx = torch.arange(Nx, device=device)
    
    # Tridiagonal Laplacian components
    A[idx, idx] = 1.0 + 2.0 * alpha
    A[idx[:-1], idx[1:]] = -alpha
    A[idx[1:], idx[:-1]] = -alpha
...
# =============================================================================
# 5. MAIN SIMULATION LOOP (HYBRID SOLVER)
# =============================================================================
...
def run_bicrystal_1d(output_dir, delta_g_dop_eV, delta_g_vo_eV=-1.5, N_vo_c_phys=1.0e27, N_dop_c_phys=1.68e26,
    ...
    eps0, eps_r, F, e_charge = 8.854e-12, 56.0, 96485.0, 1.6022e-19
    ...
    D_vo_dim = 1.0
    D_dop_dim = (D_dop_phys / D_vo_phys) 
    L_mobility = 1.0 
    gamma = 1.5
    ...
    print(f"--- Setup 1D Benchmark (Hybrid Solver) ---")
    ...
    # PHASE 1: Form the structural Phase-Field Geometry (Only 25,000 steps!)
    steps = 25001 
    nt = 5000 
    ...
    for n in range(steps):
        ...
        if n % nt == 0:
            ...
            sig_dim, wid_dim = compute_properties_1d(eta, dx, omega, gamma, kappa)
            sig_phys_val = (sig_dim * energy_scale * length_scale).item()
            wid_phys_val = (wid_dim * length_scale).item()
            print(f"Forming Structure Step {n}: Max Phi={np.max(phi_phys):.4f} V, Max Vo={np.max(C_vo_phys):.4e} | Sig={sig_phys_val:.4f} J/m^2 | Width={wid_phys_val*1e9:.4f} nm")


    # =========================================================================
    # PHASE 2: DIRECT EQUILIBRIUM SOLVER
    # =========================================================================
    print("\n>>> Phase-field geometry relaxed. Switching to Direct Equilibrium Solver! <<<")
    
    # Extract the final relaxed shape h(x)
    h, _ = compute_h_and_derivative_1d(eta)
    h_np = h.cpu().numpy()
    ...
    # FIXED: Added / Vt to convert eV into dimensionless thermal energy!
    K_vo = np.exp(-delta_g_vo_eV / Vt)
    K_dop = np.exp(-delta_g_dop_eV / Vt)

    def get_concentrations_eq(phi_array):
        # Fermi-Dirac Bulk
        ...
        # Interpolation
        C_vo_res = (1.0 - h_np) * C_vo_b + h_np * C_vo_c
        C_dop_res = (1.0 - h_np) * C_dop_b + h_np * C_dop_c
        return C_vo_res, C_dop_res

    def poisson_residual(phi_array):
        C_vo_res, C_dop_res = get_concentrations_eq(phi_array)
        rho = e_charge * (z_vo * C_vo_res + z_dop * C_dop_res)
        
        dx_m = dx * length_scale
        res = np.zeros_like(phi_array)
        
        # Interior points scaled by dx^2 to prevent Krylov Jacobian explosion
        res[1:-1] = (phi_array[:-2] - 2.0 * phi_array[1:-1] + phi_array[2:]) + (rho[1:-1] / (eps0 * eps_r)) * (dx_m**2)
        
        # Boundaries: Both ends of the 800nm domain are deep bulk (0.0 Volts)
        res[0] = phi_array[0] - 0.0
        res[-1] = phi_array[-1] - 0.0
        
        return res

    # Solve instantly using the last dynamic potential as an excellent initial guess!
    phi_guess = (phi * phi_scale).cpu().numpy()
    sol = root(poisson_residual, phi_guess, method='krylov', tol=1e-8)
    
    if sol.success:
        print("Equilibrium solved successfully!")
    else:
        print("Warning: Equilibrium solver struggled, check parameters.")

    # Overwrite the dynamic physical variables with the exact analytical equilibrium results
    phi_phys = sol.x
    C_vo_phys, C_dop_phys = get_concentrations_eq(phi_phys)
    rho_phys = e_charge * (z_vo * C_vo_phys + z_dop * C_dop_phys)
    
    print(f"\nFINAL EQUILIBRIUM:\nMax Phi={np.max(phi_phys):.4f} V, Max Vo={np.max(C_vo_phys):.4e} 1/m3, Max Dop={np.max(C_dop_phys):.4e} 1/m3\n")

    # =========================================================================
    # PLOTTING & SAVING
    # =========================================================================
    # Save Final Equilibrium to HDF5
    h5_path = os.path.join(output_dir, f"results_data.h5")
    with h5py.File(h5_path, "w") as h5file:
        grp = h5file.create_group(f"equilibrium_state") 
        grp["x_nm"] = x_nm 
    ...
    plt.plot(x_nm, phi_phys, 'k-', lw=2.5, label='Phase-Field Equilibrium (Diffuse)')
    plt.plot(x_nm, phi_gc_theory, 'r--', lw=2, label='Gouy-Chapman (Sharp Interface)')
    plt.title("Electrostatic Potential Comparison (Exact Equilibrium)")
